...
vector< vector<float> > normalize(vector< vector <float> > grid) {
  ...
  for( row =0 ; row < grid.size() ; row++)
  {
    ...
    for( col =0 ; col < grid[0].size() ; col++)
   	{
       new_row.push_back( grid[row][col]/total); 
  	}
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
  ...
  int i,j;
 	...
 	int grid_val =0,mult =0,new_i =0 , new_j =0 , dx=0 ,dy =0;
  
  	vector < vector <float> > newGrid(height,vector <float>(width,0));
  	vector < vector <float> > window(height,vector <float>(width,0));
