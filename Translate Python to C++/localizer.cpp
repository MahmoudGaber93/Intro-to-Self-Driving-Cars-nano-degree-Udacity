...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	return vector< vector <float> >(height,vector <float>(width,belief_per_cell));
}
	...
	float p_miss) 
{
  ...
  for( row = 0 ; row <  height ; row++)
  {
        for( col = 0 ; col <  width ; col++)
        {
            ...
            {
                newGrid[row][col] = beliefs[row][col]* p_hit;
            }
                
            else
            {
				newGrid[row][col] = beliefs[row][col]* p_miss;
            }
        }
  }
